...
if (!(Test-Path -LiteralPath $ModelPbix)) {
  Write-Validation @{
    ...
    reason = "No output/dashboard_model.pbix exists. Open/save the PBIP once or copy a valid model PBIX before applying native report layout."
  } 2
}
...
if (!(Test-Path -LiteralPath $LayoutJson)) {
  Write-Validation @{
    ...
    final_pbix = $FinalPbix
    final_pbix_created = $false
    reason = "Native report layout JSON is missing. Run build/scripts/10_build_native_pbix_report.py first."
  } 2
}
...
try {
  ...
  if (!(Test-Path -LiteralPath $PackagingDll)) {
    Write-Validation @{
      ...
      layout_json = $LayoutJson
      final_pbix_created = $false
      reason = "Microsoft.PowerBI.Packaging.dll was not found, so the PBIX package cannot be validated after patching."
    } 2
  }
  ...
  try {
    ...
    if (-not $package.PartExists($layoutUri)) {
      throw "The source model PBIX does not contain /Report/Layout."
    }
  ...
  Write-Validation @{
    ...
    validation = "PowerBIPackager.Validate passed for output and final PBIX after native report layout patch."
  }
}
catch {
  Write-Validation @{
