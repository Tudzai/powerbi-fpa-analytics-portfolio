param(
  ...
  [string]$ModelPbix = "C:\Users\Win\OneDrive\Codex\Portfolio\BI\Project 10 - AML Fraud Monitoring\output\dashboard_model_clean.pbix",
  [string]$LayoutJson = "C:\Users\Win\OneDrive\Codex\Portfolio\BI\Project 10 - AML Fraud Monitoring\build\legacy_report_layout_aml.json",
...
$metadata = [ordered]@{
  ...
  report_layout_format = "legacy Report/Layout"
  security_bindings_removed = $true
}

$metadata | ConvertTo-Json -Depth 10 | Set-Content -LiteralPath (Join-Path $ProjectRoot "qa\pbix_legacy_report_validation.json") -Encoding UTF8
