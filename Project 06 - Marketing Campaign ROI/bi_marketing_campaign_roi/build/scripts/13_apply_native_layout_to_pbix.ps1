...
if (!(Test-Path -LiteralPath $ModelPbix)) {
  Write-Validation @{
    ...
    reason = "No valid model PBIX exists. Run build/scripts/12_push_model_bim_via_tom.ps1 and save dashboard_model_seed.pbix first."
  } 2
}
...
if (!(Test-Path -LiteralPath $LayoutJson)) {
  Write-Validation @{
    ...
    layout_json = $LayoutJson
    final_pbix_created = $false
    reason = "Native report layout JSON is missing. Run build/scripts/10_build_powerbi_pbip_and_layout.py first."
  } 2
}
...
try {
  ...
  Write-Validation @{
    ...
    validation = "PowerBIPackager.Validate passed for output and final PBIX after Project 06 - Marketing Campaign ROI native report layout patch."
  }
}
catch {
  Write-Validation @{
