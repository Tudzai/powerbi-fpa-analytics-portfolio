...
function Get-TableByName($Model, [string]$Name) {
  foreach ($table in $Model.Tables) {
    if ($table.Name -eq $Name) { return $table }
  }
...
foreach ($tableDef in $modelDefinition.model.tables) {
  ...
  if ([string]($tableDef.name) -eq "KPI_Measures") {
    $partition = New-Object Microsoft.AnalysisServices.Tabular.Partition
    $partition.Name = "KPI_Measures-Import"
    ...
    $source.Expression = 'let Source = #table(type table [MeasureName = text], {{"KPI"}}) in Source'
  ...
  $measureDefs = @($tableDef.measures) | Where-Object {
    $_ -and -not [string]::IsNullOrWhiteSpace([string]($_.name))
  }
  foreach ($measureDef in $measureDefs) {
...
$relationshipDefs = @($modelDefinition.model.relationships) | Where-Object {
  $_ -and -not [string]::IsNullOrWhiteSpace([string]($_.fromTable)) -and -not [string]::IsNullOrWhiteSpace([string]($_.toTable))
}
if ($relationshipDefs.Count -eq 0) {
  ($modelDefinition.model.relationships | ConvertTo-Json -Depth 10) | Set-Content -LiteralPath (Join-Path $QaRoot "relationship_debug.json") -Encoding UTF8
  throw "No non-empty relationship definitions found in model.bim."
}

foreach ($relationshipDef in $relationshipDefs) {
...
$result = [ordered]@{
  ...
  status = "project6_model_pushed_via_tom"
